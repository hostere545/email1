(function () {
	...
	function pluginData() {
		...
		Array.from(choicesExamples).forEach(function (item) {
			// Skip if Choices is already initialized on this element
			if (item.hasAttribute('data-choices-initialized')) {
				return;
			}
			...
			// Initialize Choices and mark element as initialized
			if (isChoicesVal["data-choices-text-disabled-true"]) {
				new Choices(item, choiceData).disable();
			} else {
				new Choices(item, choiceData);
			}
			item.setAttribute('data-choices-initialized', 'true');
		});
	...
	function twoColumnMenuGenerate() {
		...
		if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
			...
			ul.innerHTML = '<a href="#" class="logo"><img src="../../../assets/images/logo/tawbasoft-favicon.png" alt="favicon" height="40"></a>';
	...
	// 						var modalCloseBtn = document.getElementById("NotificationModalbtn-close");
	// 						if (modalCloseBtn) {
	// 							modalCloseBtn.click();
	// 						}
	// 					})
			...
			case isLayoutAttributes:
				...
				switch (isLayoutAttributes["data-sidebar-image"]) {
					...
					case "img-4":
						...
						break;
					case "img-5":
						getElementUsingTagname("data-sidebar-image", "img-5");
						sessionStorage.setItem("data-sidebar-image", "img-5");
						document.documentElement.setAttribute("data-sidebar-image", "img-5");
						break;
					default:
						if (sessionStorage.getItem("data-sidebar-image") && sessionStorage.getItem("data-sidebar-image") == "none") {
							...
							getElementUsingTagname("data-sidebar-image", "img-4");
							document.documentElement.setAttribute("data-sidebar-image", "img-4");
						} else if (sessionStorage.getItem("data-sidebar-image") == "img-5") {
							sessionStorage.setItem("data-sidebar-image", "img-5");
							getElementUsingTagname("data-sidebar-image", "img-5");
							document.documentElement.setAttribute("data-sidebar-image", "img-5");
						}
	...
	function setDefaultAttribute() {
		if (!sessionStorage.getItem("defaultAttribute")) {
			...
			var offCanvas = document.querySelector('.off[data-bs-target="#theme-settings-offcanvas"]');
...
// Removed undefined myObject reference to prevent ReferenceError
	// console.log("O,O Morsid o");
}
